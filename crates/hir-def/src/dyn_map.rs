...
pub mod keys {
    ...
    use crate::{
        ...
        dyn_map::{Key, ValueTrait, declare_keys},
    };
    ...
    declare_keys! {
        ...
        pub const for<'db> DERIVE_MACRO_CALL<
            ast::Meta,
            (
                AttrId,
                /* derive() */ MacroCallId,
...
macro_rules! declare_keys {
    {
        $vis:vis const for<$db_lt:lifetime> $key_name:ident<$key:ty, $value:ty $(,)?>;
        $( $rest:tt )*
    } => {
    ...
    };
    {
        ...
        declare_keys! {
            $vis const for<'db> $key_name<$key, $value>;
            $( $rest )*
        }
    };
    // Recursion base case.
