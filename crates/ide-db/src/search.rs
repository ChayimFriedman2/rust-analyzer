...
use hir::{
    sym, Adt, AsAssocItem, DefWithBody, DescendPreference, FileRange, FileRangeWrapper, HasAttrs,
    HasContainer, HasSource, HirFileIdExt, InFile, InFileWrapper, InRealFile, ItemContainer,
...
impl<'a> FindUsages<'a> {
    ...
    fn short_associated_function_fast_search(
        ...
        fn collect_possible_aliases(
            ...
            while let Some((current_to_process, current_to_process_search_scope)) = to_process.pop()
            {
                let is_alias = |alias: &ast::TypeAlias| {
                    ...
                    let ty = def.ty(db);
                    if ty.is_non_opaque_type_alias() {
                        // A non-resolved type alias can refer to any type, including our type (to be precise,
                        // any type mentioned in its declaration or in its instantiation).
                        // So be safe and include it, and let's just hope there are few unresolved type aliases
                        // that mention our name (spoiler alert: this is probably true).
                        return Some(def);
                    }
                ...
                if total_files_searched > 20_000 && completed.len() > 100 {
                    // This case is very unlikely, but if we get here, we at a risk of entering an almost-infinite
                    // loop of growing the aliases list. So just stop and let normal search handle this case.
        ...
        let _p = tracing::info_span!("Self_search").entered();
        search(
            self,
            &finder,
            name,
        ...
        drop(_p);
        let _p = tracing::info_span!("aliases_search").entered();
        search(
            self,
            &finder,
            name,
