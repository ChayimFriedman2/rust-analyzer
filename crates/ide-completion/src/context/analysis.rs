...
pub(super) fn expand_and_analyze(
    ...
    let offset = expansion.original_offset + relative_offset;
    ...
    analyze(sema, expansion, original_token, &token).map(|(analysis, expected, qualifier_ctx)| {
        AnalysisResult { analysis, expected, qualifier_ctx, token, original_offset: offset }
    })
}
...
/// that we check, we subtract `COMPLETION_MARKER.len()` but only if it contains the text of
/// `COMPLETION_MARKER` (because proc macros can make the same span with different text). This may not be
/// accurate because proc macros can insert the text of the completion marker in other places while
/// removing the span, but this is the best we can do.
fn expand(
    sema: &Semantics<'_, RootDatabase>,
    original_file: SyntaxNode,
    speculative_file: SyntaxNode,
...
fn classify_name_ref(
    ...
    if path_ctx.is_trivial_path() {
        ...
        let top_node = match path_ctx.kind {
            ...
            PathKind::Item { .. } => {
                parent.ancestors().find(|it| ast::ItemRecovery::can_cast(it.kind()))
            }
            _ => None,
        };
