...
pub(crate) fn complete_dot(
    ...
    if let Some(future_output) = receiver_ty.into_future_output(ctx.db) {
        ...
        complete_fields(
            ...
            ctx,
            &future_output,
            |acc, field, ty| acc.add_field(ctx, &dot_access, Some(sym::await_.clone()), field, &ty),
            |acc, field, ty| acc.add_tuple_field(ctx, Some(sym::await_.clone()), field, &ty),
            is_field_access,
            is_method_access_with_parens,
        );
        complete_methods(ctx, &future_output, &traits_in_scope, |func| {
            acc.add_method(ctx, &dot_access, func, Some(sym::await_.clone()), None)
        });
    }
    ...
    let iter = receiver_ty
        ...
        .map(|ty| (ty, sym::iter_call.clone()))
        .or_else(|| {
            receiver_ty
                .clone()
                .into_iterator_iter(ctx.db)
                .map(|ty| (ty, sym::into_iter_call.clone()))
        });
    ...
    complete_fields(
        ...
        |acc, field, ty| {
            acc.add_field(
                ...
                Some(sym::self_.clone()),
                field,
                &ty,
            )
        },
        |acc, field, ty| acc.add_tuple_field(ctx, Some(sym::self_.clone()), field, &ty),
        true,
        false,
    );
