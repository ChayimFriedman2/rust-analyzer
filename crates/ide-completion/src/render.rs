...
pub(crate) fn render_field(
    ...
    let mut item = CompletionItem::new(
        SymbolKind::Field,
        ctx.source_range(),
        field_with_prefix(prefix.as_ref(), &name),
    ...
    if !is_field_access || ty.is_fn() || ty.is_closure() {
        ...
        builder
            .replace(ctx.source_range(), field_with_prefix(prefix.as_ref(), &escaped_name).into());
        ...
        item.insert_text(field_with_prefix(prefix.as_ref(), &escaped_name));
    }
    if let Some(receiver) = &dot_access.receiver {
...
fn field_with_prefix(prefix: Option<&hir::Symbol>, field_name: &str) -> SmolStr {
    prefix
...
pub(crate) fn render_tuple_field(
    ...
    let mut item = CompletionItem::new(
        SymbolKind::Field,
        ctx.source_range(),
        field_with_prefix(prefix.as_ref(), &field.to_string()),
        ctx.completion.edition,
    );
