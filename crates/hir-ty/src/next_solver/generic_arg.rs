...
use rustc_type_ir::{
    ClosureArgs, ConstVid, CoroutineArgs, CoroutineClosureArgs, CustomizableTypeVisitable,
    FallibleTypeFolder, FnSigTys, Interner, TyKind, TyVid, TypeFoldable, TypeFolder, TypeVisitable,
...
impl<'db, V> CustomizableTypeVisitable<V> for GenericArg<'db>
where
    GenericArgKind<'db>: CustomizableTypeVisitable<V>,
{
...
impl<'db, V> CustomizableTypeVisitable<V> for Term<'db>
where
    TermKind<'db>: CustomizableTypeVisitable<V>,
{
    fn customizable_visit_with(&self, visitor: &mut V) {
        self.kind().customizable_visit_with(visitor);
    }
