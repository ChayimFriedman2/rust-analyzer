...
use macros::CustomizableTypeVisitable;
...
use rustc_type_ir::{
    AliasTermKind, AliasTyKind, BoundVar, CollectAndApply, CoroutineWitnessTypes,
    CustomizableTypeVisitable, DebruijnIndex, EarlyBinder, FlagComputation, Flags, GenericArgKind,
    ImplPolarity, InferTy, Interner, TraitRef, TypeFlags, TypeVisitableExt, UniverseIndex, Upcast,
    Variance,
    elaborate::elaborate,
    error::TypeError,
    fast_reject,
...
macro_rules! interned_slice {
    ($storage:ident, $name:ident, $ty_db:ty, $ty_static:ty $(,)?) => {
        ...
        impl<'db, V: $crate::next_solver::interner::WorldExposer>
            rustc_type_ir::CustomizableTypeVisitable<V> for $name<'db>
        {
            #[inline]
            fn customizable_visit_with(&self, visitor: &mut V) {
                if visitor.on_interned_slice(self.interned).is_continue() {
                    self.as_slice().iter().for_each(|it| it.customizable_visit_with(visitor));
                }
            }
        }
    };
...
impl<'db, V: WorldExposer> rustc_type_ir::CustomizableTypeVisitable<V> for Pattern<'db> {
    fn customizable_visit_with(&self, visitor: &mut V) {
...
impl<'db> Interner for DbInterner<'db> {
    ...
    fn require_trait_lang_item(self, lang_item: SolverTraitLangItem) -> TraitIdWrapper {
        ...
        let lang_item = match lang_item {
            ...
            SolverTraitLangItem::TrivialClone => unimplemented!(),
        };
        lang_item.expect("Lang item required but not found.").into()
    }
...
macro_rules! TrivialTypeTraversalImpls {
    ($($ty:ty,)+) => {
        $(
            ...
            impl<V> rustc_type_ir::CustomizableTypeVisitable<V> for $ty {
                #[inline]
        ...
        $(
            impl ::intern::GcInternedVisit for $ty {
                ...
                fn visit_with(&self, gc: &mut ::intern::GarbageCollector) {
                    self.customizable_visit_with(gc);
                }
            }
        )*
    };
        ...
        $(
            impl ::intern::GcInternedSliceVisit for $ty {
                ...
                fn visit_header(header: &<Self as ::intern::SliceInternable>::Header, gc: &mut ::intern::GarbageCollector) {
                    header.customizable_visit_with(gc);
                }
