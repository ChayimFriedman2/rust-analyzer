...
use rustc_type_ir::{
    ...
    Upcast,
    inherent::{AdtDef, GenericArgs as _, SliceLike, Ty as _},
};
...
use crate::{
    ...
    mir::pad16,
    next_solver::{
        AliasTy, Clause, ClauseKind, ConstKind, DbInterner, EarlyBinder, ExistentialPredicate,
        FnSig, GenericArg, GenericArgKind, GenericArgs, PolyFnSig, Region, SolverDefId, Term,
        TermKind, TraitRef, Ty, TyKind, TypingMode,
        abi::Safety,
        ...
        let bounds = GenericPredicates::query_all(f.db, param.id.parent())
            ...
            .filter(|wc| {
                let ty = match wc.kind_skip_binder() {
                    ...
                    ClauseKind::TypeOutlives(t) => t.0.clone(),
                    _ => return false,
                };
        ...
        if !bounds.is_empty() {
            return f.format_bounds_with(alias.clone(), |f| {
                write_bounds_like_dyn_trait_with_prefix(
                    f,
                    "impl",
                    Either::Left(Ty::new_alias(AliasTyKind::Projection, alias.clone())),
                    &bounds,
...
impl<'db> HirDisplay<'db> for crate::next_solver::Const<'db> {
    fn hir_fmt(&self, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
        match self.kind() {
...
fn render_const_scalar_inner<'db>(
    ...
    match ty.kind() {
        ...
        TyKind::Array(ty, len) => {
            let Some(len) = consteval::try_const_usize(f.db, len.clone()) else {
    ...
    match data.shape {
        FieldsShape::Record | FieldsShape::Tuple => {
            let render_field = |f: &mut HirFormatter<'_, 'db>, id: LocalFieldId| {
                ...
                let ty = field_types[id].clone().instantiate(f.interner, &args);
                let Ok(layout) = f.db.layout_of_ty(ty.clone(), trait_env.clone()) else {
                    return f.write_str("<layout-error>");
                };
                let size = layout.size.bytes_usize();
    ...
    fn hir_fmt(
        ...
        match self.kind() {
                ...
                let (preds_to_print, has_impl_fn_pred) = match t.kind() {
                    ...
                    TyKind::Alias(AliasTyKind::Opaque, ty) => {
                        ...
                        if let ImplTraitId::ReturnTypeImplTrait(func, _) = impl_trait_id {
                            ...
                            let bounds = || data.iter_instantiated_cloned(f.interner, &ty.args);
            ...
            TyKind::Adt(def, parameters) => {
                ...
                hir_fmt_generics(f, parameters, Some(def.def_id().0.into()), None)?;
            }
            ...
            TyKind::Alias(AliasTyKind::Opaque, alias_ty) => {
                ...
                let data = impl_trait_id.predicates(db);
                let bounds =
                    data.iter_instantiated_cloned(interner, &alias_ty.args).collect::<Vec<_>>();
            ...
            TyKind::Closure(id, substs) => {
                ...
                match f.closure_style {
                    ...
                    ClosureStyle::ClosureWithSubst => {
                        ...
                        return hir_fmt_generics(f, substs, None, None);
                    }
                    _ => (),
                }
                let sig = substs.clone().signature_unclosure(Safety::Safe);
                let sig = sig.skip_binder();
            ...
            TyKind::CoroutineClosure(id, args) => {
                ...
                match f.closure_style {
                    ...
                    ClosureStyle::ClosureWithSubst => {
                        ...
                        return hir_fmt_generics(f, args, None, None);
                    }
                    _ => (),
                }
                ...
                let coroutine_output = coroutine_sig.output();
                let TyKind::Tuple(coroutine_output) = coroutine_output.kind() else {
                    unreachable!("invalid coroutine closure signature");
                };
                let coroutine_output = coroutine_output.as_slice()[1].clone();
                match f.closure_style {
                    ClosureStyle::ImplFn => write!(f, "impl {kind}(")?,
                ...
                if coroutine_inputs.is_empty() {
                    ...
                    f.write_joined(coroutine_inputs.iter(), ", ")?;
                };
                match f.closure_style {
                    ClosureStyle::ImplFn => write!(f, ")")?,
                    ClosureStyle::RANotation => write!(f, "|")?,
            ...
            TyKind::Dynamic(bounds, region) => {
                ...
                for bound in bounds.as_slice() {
                    let clause = bound.with_self_ty(interner, self.clone());
                ...
                if f.render_region(region) {
                    bounds_to_display.push(
                        rustc_type_ir::OutlivesPredicate(self.clone(), region.clone())
                            .upcast(interner),
                    );
                }
...
fn write_bounds_like_dyn_trait<'db>(
    ...
    for p in predicates {
        match p.kind().skip_binder() {
            ...
            ClauseKind::TypeOutlives(to) if Either::Left(to.0.clone()) == this => {
                if !is_fn_trait && angle_open {
                    write!(f, ">")?;
                    angle_open = false;
                }
            ...
            ClauseKind::RegionOutlives(lo) if Either::Right(lo.0.clone()) == this => {
                if !is_fn_trait && angle_open {
                    write!(f, ">")?;
                    angle_open = false;
                }
            ...
            ClauseKind::Projection(projection) if is_fn_trait => {
                is_fn_trait = false;
                if !projection.term.ty().is_some_and(|it| it.is_unit()) {
                    write!(f, " -> ")?;
