...
use ide_db::{
    base_db::{CrateId, SourceDatabase},
    FileId, RootDatabase,
};
...
pub(crate) fn discover_test_roots(db: &RootDatabase) -> Vec<TestItem> {
    db.all_crates()
        .iter()
        .copied()
        .filter(|&id| db.crate_data(id).origin.is_local())
        .filter_map(|id| {
            let test_id = db.extra_crate_data(id).display_name.as_ref()?.to_string();
            Some(TestItem {
...
fn find_crate_by_id(db: &RootDatabase, crate_id: &str) -> Option<CrateId> {
    ...
    db.all_crates().iter().copied().find(|&id| {
        db.crate_data(id).origin.is_local()
            && db
                .extra_crate_data(id)
                .display_name
                .as_ref()
...
pub(crate) fn discover_tests_in_crate(db: &RootDatabase, crate_id: CrateId) -> Vec<TestItem> {
    if !db.crate_data(crate_id).origin.is_local() {
        return vec![];
    }
    let Some(crate_test_id) = &db.extra_crate_data(crate_id).display_name else {
        return vec![];
    };
