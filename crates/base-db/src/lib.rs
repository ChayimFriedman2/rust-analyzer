...
pub use crate::{
    change::FileChange,
    input::{
        BuiltProcMacroPaths, CrateBuilder, CrateBuilderId, CrateData, CrateDataBuilder,
        CrateDisplayName, CrateGraphBuilder, CrateId, CrateName, CrateOrigin, CratesIdMap,
        Dependency, DependencyBuilder, Env, ExtraCrateData, LangCrateOrigin, ProcMacroPaths,
        ReleaseChannel, SourceRoot, SourceRootId, TargetLayoutLoadResult, UniqueCrateData,
    },
};
...
pub fn transitive_deps(db: &dyn SourceDatabase, crate_id: CrateId) -> FxHashSet<CrateId> {
    ...
    while let Some(krate) = worklist.pop() {
        ...
        worklist.extend(db.crate_data(crate_id).dependencies.iter().map(|dep| dep.crate_id));
    }
