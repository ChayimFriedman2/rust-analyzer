...
use crate::{CrateWorkspaceData, SourceDatabase, SourceRootDatabase};
...
pub struct CrateId(ra_salsa::InternId);

impl ra_salsa::InternKey for CrateId {
...
/// The crate data from which we derive the `CrateId`.
///
...
pub struct UniqueCrateData {
    root_file_id: FileId,
    disambiguator: Option<(Arc<CrateData>, Arc<HashableCfgOptions>)>,
}

impl ra_salsa::InternValueTrivial for UniqueCrateData {}

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct CrateDataGeneric<Id> {
    // We store the root file here too (and not only in `UniqueCrateData`) because it's convenient and saves
...
pub type CrateData = CrateDataGeneric<CrateId>;
...
pub type Dependency = DependencyGeneric<CrateId>;
...
pub type CratesIdMap = FxHashMap<CrateBuilderId, CrateId>;
...
impl CrateGraphBuilder {
    ...
    pub fn add_dep(
        ...
        if let Some(path) = self.find_path(&mut FxHashSet::default(), dep.crate_id, from) {
            let path = path
                .into_iter()
                .map(|it| (it, self.arena[it].extra.display_name.clone()))
                .collect();
    ...
    pub fn set_in_db(self, db: &mut dyn SourceRootDatabase) -> CratesIdMap {
        ...
        // salsa doesn't compare new input to old input to see if they are the same, and also doesn't
        // have a way to fallibly request input, so here we are doing all the work ourselves.
        ...
        fn go(
            ...
            db: &mut dyn SourceRootDatabase,
            old_all_crates: &FxHashSet<CrateId>,
            visited: &mut FxHashMap<CrateBuilderId, CrateId>,
            visited_root_files: &mut FxHashSet<FileId>,
            all_crates: &mut Vec<CrateId>,
            source: CrateBuilderId,
        ) -> CrateId {
            if let Some(&crate_id) = visited.get(&source) {
                return crate_id;
            }
            let dependencies = graph.arena[source]
                .basic
                .dependencies
                .iter()
                .map(|dep| Dependency {
                ...
                .collect::<Vec<_>>();
            let krate = &graph.arena[source];
            let crate_data = Arc::new(CrateData {
                dependencies,
            ...
            let disambiguator = if visited_root_files.insert(krate.basic.root_file_id) {
                ...
                Some((crate_data.clone(), Arc::new(krate.cfg_options.to_hashable())))
            };
            let crate_id = db.intern_unique_crate_data(UniqueCrateData {
                root_file_id: krate.basic.root_file_id,
                disambiguator,
            });
            let crate_already_exists = old_all_crates.contains(&crate_id);
            if !crate_already_exists || crate_data != db.crate_data(crate_id) {
                db.set_crate_data_with_durability(crate_id, crate_data, Durability::HIGH);
            }
            if !crate_already_exists || krate.extra != *db.extra_crate_data(crate_id) {
                db.set_extra_crate_data_with_durability(
                    crate_id,
                    Arc::new(krate.extra.clone()),
                    Durability::HIGH,
                );
            }
            if !crate_already_exists || krate.cfg_options != db.crate_cfg(crate_id) {
                db.set_crate_cfg_with_durability(
                    crate_id,
                    krate.cfg_options.clone(),
                    Durability::HIGH,
                );
            }
            if !crate_already_exists || krate.env != db.crate_env(crate_id) {
                db.set_crate_env_with_durability(crate_id, krate.env.clone(), Durability::HIGH);
            }
            if !crate_already_exists || krate.ws_data != db.crate_workspace_data(crate_id) {
                db.set_crate_workspace_data_with_durability(
                    crate_id,
                    krate.ws_data.clone(),
                    Durability::HIGH,
                );
            }
            all_crates.push(crate_id);
            visited.insert(source, crate_id);
            crate_id
        }
    }
    ...
    pub fn transitive_deps(&self, of: CrateBuilderId) -> impl Iterator<Item = CrateBuilderId> {
        ...
        while let Some(krate) = worklist.pop() {
            ...
            worklist.extend(self.arena[krate].basic.dependencies.iter().map(|dep| dep.crate_id));
        }
    ...
    fn find_path(
        ...
        for dep in &self.arena[from].basic.dependencies {
...
pub(crate) fn transitive_rev_deps(db: &dyn SourceDatabase, of: CrateId) -> FxHashSet<CrateId> {
    let mut worklist = vec![of];
...
mod tests {
    ...
    use crate::CrateWorkspaceData;

    use super::{
        CrateGraphBuilder, CrateName, CrateOrigin, DependencyGeneric, Edition::Edition2018, Env,
        FileId,
    };
    ...
    fn detect_cyclic_dependency_indirect() {
        ...
        assert!(graph
            .add_dep(crate1, DependencyGeneric::new(CrateName::new("crate2").unwrap(), crate2,))
            .is_ok());
        assert!(graph
            .add_dep(crate2, DependencyGeneric::new(CrateName::new("crate3").unwrap(), crate3,))
            .is_ok());
        assert!(graph
            .add_dep(crate3, DependencyGeneric::new(CrateName::new("crate1").unwrap(), crate1,))
            .is_err());
    }
    ...
    fn detect_cyclic_dependency_direct() {
        ...
        assert!(graph
            ...
            .is_ok());
        assert!(graph
            .add_dep(crate2, DependencyGeneric::new(CrateName::new("crate2").unwrap(), crate2,))
            .is_err());
    }
    ...
    fn dashes_are_normalized() {
        ...
        assert_eq!(
            ...
            vec![
                DependencyGeneric::new(CrateName::new("crate_name_with_dashes").unwrap(), crate2,)
            ]
        );
    }
}
